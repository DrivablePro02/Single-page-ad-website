...
const FeatureCard = ({icon, title, content, index}) => (
  <div className={`flex flex-row p-6 rounded-[20px] ${index !== features.length - 1 ? "mb-6" : "mb-0" } feature-card`}>
    <div className={`w-[64px] h-[64px] rounded-full ${styles.flexCenter} cursor-pointer bg-dimBlue `}>
    ...
    <div className='flex-1 flex flex-col ml-3'>
      ...
      <p className={`text-dimWhite max-w-[470px] font-poppins font-normal text-[16px] leading-[24px] mb-1 `}>{content}</p>
    </div>
  </div>
)
